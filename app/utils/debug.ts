...
export const showDatabaseInfo = async () => {
  try {
    ...
    Alert.alert(
      "Erro",
      `Não foi possível obter informações do banco de dados: ${String(error)}`
    );
  }
};
...
export const showTableData = async (tableName: string) => {
  try {
    ...
    if (count === 0) {
      Alert.alert(
        "Tabela Vazia",
        `A tabela ${tableName} não possui registros.`
      );
      return;
    }
    ...
    Alert.alert(
      "Erro",
      `Não foi possível obter dados da tabela ${tableName}: ${String(error)}`
    );
  }
};
