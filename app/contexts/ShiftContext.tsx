...
interface ShiftContextData {
  ...
  updateShift: (
    data: { id: string } & Partial<Omit<Shift, 'id' | 'userId' | 'createdAt' | 'updatedAt'>>
  ) => Promise<boolean>;
...
export const ShiftProvider: React.FC<ShiftProviderProps> = ({ children }) => {
  ...
  const createShift = async (
    ...
    if (!isSignedIn || !user) throw new Error('Usuário não autenticado');
  ...
  const updateShift = async (
    ...
    try {
      const success = await shiftService.updateShift(data.id, data as ShiftUpdateData);
  ...
  const getShiftsByMonth = async (year: number, month: number): Promise<void> => {
    if (!isSignedIn || !user) return;
    try {
      setLoading(true);
      const fetchedShifts = await shiftService.getShiftsByMonth(user.id, year, month);
      setShifts(fetchedShifts);
    } catch (error) {
