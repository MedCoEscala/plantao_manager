...
export default function VerifyCodeScreen() {
  ...
  useEffect(() => {
    ...
    // DEBUG: Verificar todos os parâmetros recebidos
    console.log('🔍 [VerifyCode] Parâmetros recebidos:', {
      email: params.email,
  ...
  const handleVerifyCodeAndSync = async (verificationCode?: string) => {
    ...
    try {
      ...
      if (mountedRef.current) {
        ...
        setTimeout(() => {
          console.log(
            '🔄 [VerifyCode] Dados sincronizados, ProfileContext será atualizado automaticamente'
          );
        }, 2000);
